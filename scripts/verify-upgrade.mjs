...
import {MONTHS,LEVELS,PAYROLL_CONFIG,REFERENCE_CALCULATORS,getSessionForRole,getPayrollPayloadForRole} from '../api/_payroll-data.js';
import {RULE_SOURCE_POLICY,defaultRulesSource,withColleagueRules} from '../modules/rule-sources.js';
...
const originalPayload = getPayrollPayloadForRole('supervisor');
const payload = structuredClone(withColleagueRules(originalPayload));
assert.equal(originalPayload.config.calculators.xd.taxRate,.195);
assert.equal(payload.config.calculators.xd.taxRate,.23);
assert.deepEqual(payload.config.calculators.video,originalPayload.config.calculators.video);
for (const type of Object.keys(REFERENCE_CALCULATORS)) assert.equal(defaultRulesSource(type),'colleague');
assert.equal(defaultRulesSource('video'),'our');
const overridden = structuredClone(originalPayload);
overridden.config.calculators.level4.defaultInputs.salary = 99999;
assert.equal(withColleagueRules(overridden).config.calculators.level4.defaultInputs.salary,22240);
assert.equal(overridden.config.calculators.level4.defaultInputs.salary,99999);
...
const clamp = value => Number(value);
const ironDefaults = {...getDefaultInputs('iron'),rulesSource:defaultRulesSource('iron')};
localValues.set('zp-2-2-calculator-inputs-user-alice-iron',JSON.stringify({rulesSource:'our',actualHours:123,testsHigh:true,ratingZone:3}));
