...
assert.deepEqual(new Set(manifest.oauth_config.scopes.bot),new Set(['users:read','users.profile:read','chat:write','commands']));
...
await assert.rejects(findSlackProfileUser('test.user',deps),{code:'SLACK_CONNECTION_REQUIRED'});
...
await assert.rejects(startChallenge('test.user','192.0.2.1',{...deps,directory}),{code:'SLACK_CONNECTION_REQUIRED'});
