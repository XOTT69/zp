...
export async function findDirectoryAccount(login) {
  ...
  if (process.env.LDAP_BRIDGE_URL) {
    ...
    return validateAccount(await response.json(),login);
  }
  ...
  try {
    ...
    const result = await client.search(process.env.LDAP_BASE_DN,{scope:'sub',sizeLimit:2,
      filter:template.replaceAll('{{login}}',escapeLdapFilter(login)),attributes:['mail','displayName','sAMAccountName','userAccountControl']});
    ...
    return validateAccount({active:true,email:value('mail'),displayName:value('displayName')},login);
  } finally { await client.unbind().catch(()=>{}); }
}

function validateAccount(account,login) {
  if (account?.active !== true || typeof account.email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(account.email)) return null;
  ...
  return {login,email:account.email.toLowerCase(),displayName:String(account.displayName || login).slice(0,100)};
}
